...
public class PDFFile {
	 ...
	 *             the product or platform does not support then the exception's
	 *             {@link PDFParseException#getCause() cause} will be an instance of
	 ...
	 * need to call this.
	 * <p>
	 * Since this is the only public method for tracking down PDF objects, it is synchronized. This
	 * means that the PDFFile can only hunt down one object at a time, preventing the file's
	 * location from getting messed around.
	 * <p>
	 * This call stores the current buffer position before any changes are made and restores it
	...
	private PDFObject readDictionary(
		...
		while ((name = readObject(objNum, objGen, decrypter)) != null) {
			...
			if (name.getType() != PDFObject.NAME) {
				throw new PDFParseException("First item in dictionary must be a /Name.  (Was "
						+ name + ")");
			}
	 ...
	 * 
	 * <p>
	 * Section 3.2.3 of PDF Refernce version 1.7 defines the format of String objects. Regarding
	 * literal strings:
	 * </p>
	 * 
	 ...
	 * *
	 * <p>
	 * This only reads 8 bit basic character 'strings' so as to avoid a text string interpretation
	 ...
	 * 'text string' as per Section 3.8.1, Table 3.31 "PDF Data Types",
	 * {@link PDFStringUtil#asTextString} ()} or {@link PDFObject#getTextStringValue()} must be
	 * employed.
	 * </p>
	 * 
	 * @param objNum
		...
		if (obj != null) {
			if (obj.getType() == PDFObject.ARRAY) {
				...
				if (bounds.length == 4) {
					...
					return new Rectangle2D.Double(minX, minY, Math.abs(maxX - minX), Math.abs(maxY
							- minY));
