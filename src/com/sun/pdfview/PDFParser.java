...
/**
	...
	public int iterate() throws Exception {
		...
		if (obj instanceof Tok) {
			...
			if (cmd.equals("q")) {
				...
				// stroke the path
				if (!PDFParser.DISABLE_PATH_STROKE || (!PDFParser.DISABLE_CLIP
				...
				if (!PDFParser.DISABLE_PATH_FILL || (!PDFParser.DISABLE_CLIP
						&& this.clip == PDFShapeCmd.CLIP)) {
				...
				if (!PDFParser.DISABLE_PATH_STROKE_FILL || (!PDFParser.DISABLE_CLIP
						&& this.clip == PDFShapeCmd.CLIP)) {
				...
				debug("**** WARNING: Not handled command: " + cmd + " **************************",
						10);
				...
				if (this.catchexceptions) {
					debug("**** WARNING: Unknown command: " + cmd + " **************************",
							10);
				} else {
			...
			if (this.stack.size() != 0) {
				debug("**** WARNING! Stack not zero! (cmd=" + cmd + ", size=" + this.stack.size()
						+ ") *************************", 4);
	...
	private Object parseObject() throws PDFParseException, DebugStopException {
		...
		debug("**** WARNING! parseObject unknown token! (t.type=" + t.type
				+ ") *************************", 4);
		return null;
	}
