...
public abstract class BaseWatchable implements Watchable, Runnable {
	 ...
	 *         <li>Watchable.NEEDS_DATA if there is no data to be processed but the execution is not
	 *         yet complete
	...
	public void run() {
		...
		// System.out.println(Thread.currentThread().getName() + " exiting: status = " +
		// getStatusString());
	...
	public boolean isExecutable() {
		return ((this.status == Watchable.PAUSED || this.status == Watchable.RUNNING) && (this.gate == null || !this.gate
				.stop()));
	}
