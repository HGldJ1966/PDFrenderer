...
public class PDFRenderer extends BaseWatchable implements Runnable {
	...
	public Rectangle2D drawImage(PDFImage image) {
		...
		Rectangle r = g.getTransform().createTransformedShape(new Rectangle(0, 0, 1, 1))
				.getBounds();
		boolean isBlured = false;

		if (useBlurResizingForImages &&
				bi.getType() != BufferedImage.TYPE_CUSTOM &&
				image.getWidth() >= 1.75 * r.getWidth()
				&& image.getHeight() >= 1.75 * r.getHeight()) {
			try {
	...
	private Rectangle2D smartDrawImage(PDFImage image, BufferedImage bi, Rectangle r,
			AffineTransform at) {
		boolean isBlured = false;
		...
		if (useBlurResizingForImages &&
			...
			if (image.getWidth() > maxFactor * r.getWidth()
					&& image.getHeight() > maxFactor * r.getHeight()) {
	 ...
	 *         <li>Watchable.NEEDS_DATA when there are no commands to be processed, but the page is
	 *         not yet complete
	 *         <li>Watchable.COMPLETED when the page is done and all the commands have been
	 *         processed
