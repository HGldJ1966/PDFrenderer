...
public class PDFImage {
	...
	public static PDFImage createImage(PDFObject obj, Map<String, PDFObject> resources,
		...
		if (image.isImageMask()) {
			...
			// handled correctly.
			Color[] colors = useAsSMask ? new Color[] {
					Color.WHITE,
					Color.BLACK
			} : new Color[] {
		...
		try {
			...
			if (bi == null) {
				...
				final boolean jpegDecode = PDFDecoder
						.isLastFilter(imageObj, PDFDecoder.DCT_FILTERS);
				if (jpegDecode) {
	 ...
	 * 
	 * <p>
	 * NOTE: the color convolving is extremely slow on large images. It would be good to see if it
	...
	private static int[] createGreyToArgbMap(int numBits) {
		...
		final WritableRaster inRaster = Raster.createPackedRaster(
				...
				1, greyVals.length, new int[] {
					mask
				}, null);

		final BufferedImage greyImage = new BufferedImage(
				new PdfComponentColorModel(greyCs, new int[] {
						numBits
				}),
				inRaster, false, null);
	...
	private class JpegDecoder
	{
		 ...
		 * @throws IOException
		 *             if the image couldn't be decoded due
		...
		private BufferedImage readImage(ImageReader jpegReader, ImageReadParam param)
				throws IOException {
			if (ycckDecodeMode) {
				// The standard Oracle Java JPEG readers can't deal with CMYK YCCK encoded images
				// without a little help from us. We'll try and pick up such instances and work
				// around it.
				final IIOMetadata imageMeta = jpegReader.getImageMetadata(0);
				if (imageMeta != null) {
					final Node standardMeta = imageMeta
						...
						if (chroma != null) {
							...
							if (csType != null) {
								...
								if (csTypeNameNode != null
										&& "YCCK".equals(csTypeNameNode.getValue())) {
									// So it's a YCCK image, and we can coax a workable image out of
									// it
									// by grabbing the raw raster and installing a YCCK converting
									// color space wrapper around the existing (CMYK) color space;
									// this will
									...
									// re-calculate the color model since the color space has
									// changed
		...
		public SampleModel createCompatibleSampleModel(int width, int height) {
			...
			if (bitsPerComponent >= 8) {
				...
				default:
					...
					return new PdfSubByteSampleModel(width, height, getNumComponents(),
							bitsPerComponent);
				}
			}
		}
