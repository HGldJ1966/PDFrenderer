 ...
 * <ul>
 * <li>An explicit destination, which contains a reference to a page as well as some stuff about how
 * to fit it into the window.
 * <li>A named destination, which uses the PDF file's Dests entry in the document catalog to map a
 * name to an explicit destination
 * <li>A string destintation, which uses the PDF file's Dests entry. in the name directory to map a
 * string to an explicit destination.
 * </ul>
 * 
