...
public class DCTDecode {
	 ...
	 * <p>
	 * DCT is the format used by JPEG images, so this class simply loads the DCT-format bytes as an
	 * image, then reads the bytes out of the image to create the array. Unfortunately, their most
	 * likely use is to get turned BACK into an image, so this isn't terribly efficient... but is is
	 * general... don't hit, please.
	 * <p>
	 * The DCT-encoded stream may have 1, 3 or 4 samples per pixel, depending on the colorspace of
	 * the image. In decoding, we look for the colorspace in the stream object's dictionary to
	 * decide how to decode this image. If no colorspace is present, we guess 3 samples per pixel.
	 * 
	 * @param dict
